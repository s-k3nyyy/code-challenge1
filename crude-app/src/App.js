...
class App extends Component {
  ...
  handleCreate = (newPerson) => {
    this.setState(prevState => ({
      data: [...prevState.data, { id: Date.now(), ...newPerson }]
    }));
  };
  ...
  handleUpdate = (id, updatedData) => {
    this.setState(prevState => ({
      data: prevState.data.map(person => (person.id === id ? { ...person, ...updatedData } : person))
    }));
  };
  ...
  handleDelete = (id) => {
    this.setState(prevState => ({
      data: prevState.data.filter(person => person.id !== id)
    }));
  };
  ...
  render() {
    return (
      <div>
        ...
        <PersonForm onCreate={this.handleCreate} />
